...
- [七、出错处理与可靠性设计 (Reliability & Fault-Tolerance)](#七-出错处理与可靠性设计-reliability-&-fault-tolerance)
  - [7.1 统一捕获与响应](#71-统一捕获与响应)
  - [7.2 LLM调用容错与自动降级](#72-LLM调用容错与自动降级)
  - [7.3 RAG向量检索降级](#73-RAG向量检索降级)
  - [7.4 文件输入与代码审查防御](#74-文件输入与代码审查防御)
  - [7.5 数据库高可用与数据一致性](#75-数据库高可用与数据一致性)
...
### 七、出错处理与可靠性分析设计
#### 7.1 统一捕获与响应流程
  后端 FastAPI 配置全局异常拦截器，将常规接口错误统一包装为结构化 JSON 响应（含自定义错误码与时间戳）；流式 SSE 接口若在传输中途发生异常，自动向前端推送 event: error 事件并优雅闭合连接，前端 React 全局 ErrorBoundary 兜底防御，确保界面绝不白屏卡死。

#### 7.2 LLM调用容错与自动降级流程
  当 DeepSeek 等在线 API 出现网络抖动或 5xx 错误时，系统自动触发最多 3 次指数退避重试（1s / 2s / 4s）；若持续失败且开启了本地备用模式，系统无缝自动热切换至本地 Ollama 模型回答，保证核心问答服务持续可用。

#### 7.3 RAG向量检索降级流程
  当 ChromaDB 发生故障、检索超时（限时 2.0s）或召回内容为空时，系统自动启动平滑降级机制，跳过知识库上下文拼接，直接切回通用大模型回答，并在回答末尾标注降级友情提示，保障问答主流程不中断。

#### 7.4 文件输入与代码审查防御流程
上传文档时严格校验格式白名单（.pdf/.md/.txt）、20MB 体积限制与 SHA256 去重；提交代码审查时，静态分析器（ast / pyflakes）先拦截并精准标注语法错误行号，若遇到语法解析异常则由 LLM 直接提供基础审查兜底。

#### 7.5 数据库高可用与数据一致性流程
  SQLite 数据库开启 WAL (Write-Ahead Logging) 模式与 busy_timeout 锁等待策略，解决高并发读写死锁问题；删除文档时，后台依序同步清除 ChromaDB 向量索引、SQLite 记录及磁盘物理文件，实现“关系库 + 向量库 + 文件系统”的三维级联清理。
